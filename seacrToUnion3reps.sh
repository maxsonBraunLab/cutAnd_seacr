...
###PURPOSE: make consensus peak file from 3 replicates###
...
cmd="$BEDTOOLS intersect -a $OUT/$REF3 -b $OUT/$REF2 -wa | cut -f1-3 | sort | uniq > $OUT/$BASE\_Ref3_merge.bed"
echo "Merge -wa"
echo $cmd
eval $cmd

cmd="$BEDTOOLS intersect -a $OUT/$REF3 -b $OUT/$REF2 -wb | cut -f1-3 | sort | uniq > $OUT/$BASE\_Ref4_merge.bed"
echo "Merge -wb"
echo $cmd
eval $cmd

cmd="$BEDTOOLS intersect -a $OUT/$REF1 -b $OUT/$REF3 -wa | cut -f1-3 | sort | uniq > $OUT/$BASE\_Ref5_merge.bed"
echo "Merge -wa"
echo $cmd
eval $cmd

cmd="$BEDTOOLS intersect -a $OUT/$REF1 -b $OUT/$REF3 -wb | cut -f1-3 | sort | uniq > $OUT/$BASE\_Ref6_merge.bed"
echo "Merge -wb"
echo $cmd
eval $cmd
